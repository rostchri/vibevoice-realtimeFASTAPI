...
class Encoder(nn.Module):
    def __init__(self, types, channels, widths, kernels, strides, groups):
        super().__init__()
        ...
        for i in range(n_blocks):
            ...
            en_convs.append(
                module(
                    in_channels, out_channels, widths[i], kernels[i], strides[i], groups=groups[i]
                )
            )
...
class Decoder(nn.Module):
    def __init__(self, types, channels, widths, kernels, strides, groups, final_width):
        super().__init__()
        block_types = [XConvBlock, XDWSBlock, XMBBlocks]
        n_blocks = len(types)
        de_convs = []
...
class ULUNAS(nn.Module):
    def __init__(
        ...
        self.dpgrnn = nn.Sequential(
            *[DPGRNN(channels[-1], widths[-1], channels[-1]) for i in range(2)]
        )

        self.decoder = Decoder(
            types, channels, widths, kernels, strides, groups, final_width=erb_low + erb_high
        )
...
if __name__ == "__main__":
    ...
    macs, params = get_model_complexity_info(
        model, (16000,), as_strings=False, print_per_layer_stat=False, verbose=False
    )
    params = 0
